...
class Piece:
    ...
    def movesAll():
        ...
        [i.getMoves() if (isinstance(i, King) is False) else None for i in pieces.values()]
    ...
    def winOrDraw():
        ...
        elif (len(listW[1]) == 2 and "b" in listW[1] and len(listB[1]) ==1) or (len(listB[1]) == 2 and "b" in listB[1] and len(listW[1]) ==1):
            draw = "Insufficient Material Draw"
        elif (len(listW[1]) == 2 and "k" in listW[1] and len(listB[1]) ==1) or (len(listB[1]) == 2 and "k" in listB[1] and len(listW[1]) ==1):
            draw = "Insufficient Material Draw"
    ...
    def move(self, place):
        global whiteTurn, firstchoice, secondtile, pieces, promotion, promotionTile, draw, theEnd
        ...
        for direction in Knight.directions:
            ...
            if 0 <= x <= 7 and 0 <= y <= 7:
                if self.board[x][y] != "" and self.board[x][y] == enemyCol + "l":
                    Piece.checks[0].append([self.getPosition(), Check.PAWN]) if self.colour == 'w' else Piece.checks[1].append([self.getPosition(), Check.KNIGHT])
                elif (self.board[x][y] == "" or self.board[x][y][0] == enemyCol):
                    self.moves.append([x, y])
                else:
                    Piece.protectedPieces[0].append([x, y]) if self.colour == 'w' else Piece.protectedPieces[1].append([x, y])
...
class King(Piece):
    ...
    def getMoves(self):
        ...
        if checks:
            ...
            if self.board[self.position[0]+1][self.position[1]] == "" and self.board[self.position[0]+2][self.position[1]] == "" and self.Pieces[13].hasMoved == False:
                self.moves.append([self.position[0]+2, self.position[1]])
            if self.board[self.position[0]-1][self.position[1]] == "" and self.board[self.position[0]-2][ self.position[1]] == "" and self.board[self.position[0]-3][self.position[1]] == "" and self.Pieces[12].hasMoved == False:
                self.moves.append([self.position[0]-2, self.position[1]])
        for piece in enemymoves:
            for move in piece:
                ...
                if (a := [self.position[0] + 2, self.position[1]]) in self.moves:
                    if move == [self.position[0]+1,self.position[1]] or move == [self.position[0]+2,self.position[1]]:
                        self.moves.remove(a)
        ...
        if -1 < pos[0] - 1 < 8 and -1 < pos[1] + dir < 8:
            if self.board[pos[0] - 1][pos[1] + dir] != "" and self.board[pos[0] - 1][pos[1] + dir][0] == enemyCol:
                if (self.board[pos[0] - 1][pos[1] + dir][1] == "l"):
                    Piece.checks[0].append([pos, Check.PAWN]) if self.colour == 'w' else Piece.checks[1].append([pos, [1, - dir], 'Pawn'])
                else:
                    self.moves.append([pos[0] - 1, pos[1] + dir])
            elif self.board[pos[0] - 1][pos[1] + dir] == "" and self.board[pos[0] - 1][pos[1]] != "" and self.board[pos[0] - 1][pos[1]] == enemyCol + "p" and pieces[(pos[0] - 1, pos[1])].enPassant is True:
                self.moves.append([pos[0] - 1, pos[1] + dir])
            elif self.board[pos[0] - 1][pos[1] + dir] == "" or self.board[pos[0] - 1][pos[1] + dir][0] == self.colour:
                Piece.protectedPieces[0].append([pos[0] - 1, pos[1] + dir]) if self.colour == 'w' else Piece.protectedPieces[1].append([pos[0] - 1, pos[1] + dir])
        if -1 < pos[0] + 1 < 8 and -1 < pos[1] + dir < 8 :
            if self.board[pos[0] + 1][pos[1] + dir] != "" and self.board[pos[0] + 1][pos[1] + dir][0] == enemyCol:
                if (self.board[pos[0] + 1][pos[1] + dir][1] == "l"):
                    Piece.checks[0].append([pos, Check.PAWN]) if self.colour == 'w' else Piece.checks[1].append([pos, [-1, - dir], Check.PAWN])
                else:
                    self.moves.append([pos[0] + 1, pos[1] + dir])
            elif self.board[pos[0] + 1][pos[1] + dir] == "":
                self.moves.append([pos[0] + 1, pos[1] + dir])
            elif self.board[pos[0] + 1][pos[1]] != "" and self.board[pos[0] + 1][pos[1]] == enemyCol + "p" and pieces[(pos[0] + 1, pos[1])].enPassant is True:
                self.moves.append([pos[0] + 1, pos[1] + dir])
            elif self.board[pos[0] + 1][pos[1] + dir] == "" or self.board[pos[0] + 1][pos[1] + dir][0] == self.colour:
                Piece.protectedPieces[0].append([pos[0] + 1, pos[1] + dir]) if self.colour == 'w' else Piece.protectedPieces[1].append([pos[0] + 1, pos[1] + dir])
        if -1 < pos[1] + dir < 8 and self.board[pos[0]][pos[1] + dir] == '':
            self.moves.append([pos[0], pos[1] + dir])
            if -1 < pos[1] + 2*dir < 8 and self.board[pos[0]][pos[1] + 2*dir] == '' and (pos[1] == 1 or pos[1] == 6):
                self.moves.append([pos[0], pos[1] + 2*dir])#does not work
...
class Rook(Piece):
    ...
    def getMoves(self):
        ...
        Piece.theMoves[0].append(self.moves.copy()) if self.colour == 'w' else Piece.theMoves[1].append(self.moves.copy())
        ...
        for i in range(0, 8):
            ...
            while -1 < dummy[0] < 8 and -1 < dummy[1] < 8:
                ...
                if self.board[dummy[0]][dummy[1]] == '':
                    ...
                    Piece.protectedPieces[0].append(dummy.copy()) if self.colour == 'w' else Piece.protectedPieces[1].append(dummy.copy())
                    break
                dummy[0] += listForIteration[i][0]
                dummy[1] += listForIteration[i][1]
        Piece.theMoves[0].append(self.moves) if self.colour == 'w' else Piece.theMoves[1].append(self.moves)
...
def draw_window():
    ...
    [[pygame.draw.rect(Display, brown, (x * 100, 0 + i * 100, 100, 100)) for x in range(1, 9, 2)] for i in range(0, 8, 2)]
    [[pygame.draw.rect(Display, brown, (j * 100, 0 + i * 100, 100, 100)) for j in range(0, 8, 2)] for i in range(1, 9, 2)]
            ...
            if mousepos[0] < 800:
                if whiteWon == "" and blackWon == "" and draw == "":
                    if promotion:
                        ...
                        if promotionCoord[0] < mousepos[0] < promotionCoord[0]+100 and promotionCoord[1] < mousepos[1] < promotionCoord[1] + 100:
                            col = 'w' if promotionTile[1] == 0 else 'b'
                            if promotionCoord[0] <= mousepos[0] < promotionCoord[0] + 50 and promotionCoord[1] <= mousepos[1] < promotionCoord[1] + 50:
                                pieces[tuple(promotionTile.copy())] = Queen(col, promotionTile.copy(), board.board)
                            elif promotionCoord[0] <= mousepos[0] < promotionCoord[0] + 50 and promotionCoord[1] + 50 <= mousepos[1] < promotionCoord[1] + 100:
                                pieces[tuple(promotionTile.copy())] = Knight(col, promotionTile.copy(), board.board)
                            elif promotionCoord[0] + 50 <= mousepos[0] < promotionCoord[0] + 100 and promotionCoord[1] <= mousepos[1] < promotionCoord[1] + 50:
                                pieces[tuple(promotionTile.copy())] = Bishop(col, promotionTile.copy(), board.board)
                            elif promotionCoord[0] + 50 <= mousepos[0] < promotionCoord[0] + 100 and promotionCoord[1] + 50 <= mousepos[1] < promotionCoord[1] + 100:
                                pieces[tuple(promotionTile.copy())] = Rook(col, promotionTile.copy(), board.board)
                            promotion = False
                            whiteTurn = False if whiteTurn else True
                            Piece.movesAll()
                    elif firstchoice == 1:
                        ...
                        if secondtile != 0:
                            ...
                            if (not promotion) and a != "" and ((a[0] == 'w' and whiteTurn) or (a[0] == 'b' and not whiteTurn)):
